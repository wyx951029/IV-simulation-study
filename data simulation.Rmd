...
#  r_pp1_uc1 = mean(data[data$pp == 1 & data$X5 == 1,]$Y)
#  r_pp0_uc1 = mean(data[data$pp == 1 & data$X5 == 0,]$Y)
#  r_pp1 = mean(data[data$pp == 1,]$Y)
# r_pp0 = mean(data[data$pp == 0,]$Y)
...
#for(i in 1:10000)
#  {
#    df10_1$Y[i]=1/(1+exp(-(0.5-0.05*df10_1$trt[i]+0.03*df10_1$X1[i]-0.019*df10_1$X2[i]+0.013*df10_1$X3[i]-0.01*df10_1$X4[i]-0.03*df10_1$X5[i])))
#}
...
for (i in 1:1000){
  for(j in 1:9){
    dfi[i,j]=RMSE(df10000[[j]]$Y, ideal_model(samp(df10000[[j]]))$fitted.values)
    dfc[i,j]=RMSE(df10000[[j]]$Y, conv_model(samp(df10000[[j]]))$fitted.values)
    dfiv[i,j]=RMSE(df10000[[j]]$Y,IV_model(samp(df10000[[j]]))$fitted.values)
...
ideal_model_cr = function(data){
  ...
  coverage = ifelse(as.numeric(conf[1]) <= as.numeric(-0.01168)  & as.numeric(-0.01168) <= round(as.numeric(conf[2]),5),1,0)
  return(coverage)
}
...
conv_model_cr = function(data){
  m = lm(Y~.-patientid - physicianid - pp - X5, data = data)
  conf = confint(m, 'trt', level=0.95)
  coverage = ifelse(as.numeric(conf[1]) <= as.numeric(-0.01168)  & as.numeric(-0.01168) <= as.numeric(conf[2]),5,1,0)
  return(coverage)
}
...
IV_model_cr = function(data){
  m = ivreg(Y~.-patientid - physicianid - pp - X5| X1+X2+X3+X4+pp, data=data)
  conf = confint(m, 'trt', level=0.95)
  coverage = ifelse(as.numeric(conf[1]) <= as.numeric(-0.01168)  & as.numeric(-0.01168) <= as.numeric(conf[2]),1,0)
  return(coverage)
}
...
for (i in 1:1000){
  for (j in 1:9){
    coi[i,j] = ideal_model_cr(samp(df10000[[j]]))
    coc[i,j] = conv_model_cr(samp(df10000[[j]]))
    coiv[i,j] = IV_model_cr((samp(df10000[[j]])))
...
cr = as.data.frame(cr)
row.names(cr)=c("pp=0.3,uc=-0.07","pp=0.3,uc=-0.08","pp=0.3,uc=-0.09","pp=0.5,uc=-0.07","pp=0.5,uc=-0.08","pp=0.5,uc=-0.09","pp=0.7,uc=-0.07","pp=0.7,uc=-0.08","pp=0.7,uc=-0.09")
colnames(cr)=c("Ideal model","Conventional model","IV model")

write.csv(cr, file = "cr_10000.csv")
...
```{r}
iv10000=matrix(NA, nrow=1000, ncol=9)

for (i in 1:1000){
  for (j in 1:9){
    iv10000[i,j]=pbias(-0.01168, summary(IV_model(samp(df10000[[j]])))$coefficients[6])
  }
}

iv10000=data.frame(iv10000)
colnames(iv10000)=c("pp=0.3,uc=-0.07","pp=0.3,uc=-0.08","pp=0.3,uc=-0.09","pp=0.5,uc=-0.07","pp=0.5,uc=-0.08","pp=0.5,uc=-0.09","pp=0.7,uc=-0.07","pp=0.7,uc=-0.08","pp=0.7,uc=-0.09")

write.csv(iv10000, file="iv10000.csv")
