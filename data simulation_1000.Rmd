...
samp = function(data){
  sp=sample(1:1000,1000, replace = TRUE)
  df = data[sp,]
  return(df)
}
...
## Data Generation-Population size=1000

### Treatment Assignment
```{r,warning=FALSE}
# Patients characteristics
X1=rbinom(1000, size=1, prob=0.4) # gender
X2=rbinom(1000, size=1, prob=0.1) # age >= 75
X3=round(rnorm(1000, mean=23, sd=3)) # BMI 
X4=round(rnorm(1000, mean=130, sd=7)) # blood pressure systolic
X5=rbinom(1000, size=1, prob=0.1) # history of side effect related disease # unmeasured confounder 
...
df1000=list(df10_1,df20_1,df30_1,df10_2,df20_2,df30_2,df10_3,df20_3,df30_3)
...
for (i in 1:1000){
  for(j in 1:9){
    dfi[i,j]=RMSE(df1000[[j]]$Y, ideal_model(samp(df1000[[j]]))$fitted.values)
    dfc[i,j]=RMSE(df1000[[j]]$Y, conv_model(samp(df1000[[j]]))$fitted.values)
    dfiv[i,j]=RMSE(df1000[[j]]$Y,IV_model(samp(df1000[[j]]))$fitted.values)
...
for (i in 1:1000){
  for(j in 1:9){
    dfi[i,j]=pbias(-0.01168, summary(ideal_model(samp(df1000[[j]])))$coefficients[7])
    dfc[i,j]=pbias(-0.01168, summary(conv_model(samp(df1000[[j]])))$coefficients[6])
    dfiv[i,j]=pbias(-0.01168, summary(IV_model(samp(df1000[[j]])))$coefficients[6])
    dpb[j,1]=mean(dfi[,j])
    dpb[j,2]=mean(dfc[,j])
    dpb[j,3]=mean(dfiv[,j])
...
row.names(drmse)=c("ex=0.2,uc=-0.07","ex=0.2,uc=-0.08","ex=0.2,uc=-0.09","ex=0.3,uc=-0.07","ex=0.3,uc=-0.08","ex=0.3,uc=-0.09","ex=0.4,uc=-0.07","ex=0.4,uc=-0.08","ex=0.4,uc=-0.09")
colnames(drmse)=c("Ideal model","Conventional model","IV model")

row.names(dpb)=c("ex=0.2,uc=-0.07","ex=0.2,uc=-0.08","ex=0.2,uc=-0.09","ex=0.3,uc=-0.07","ex=0.3,uc=-0.08","ex=0.3,uc=-0.09","ex=0.4,uc=-0.07","ex=0.4,uc=-0.08","ex=0.4,uc=-0.09")
colnames(dpb)=c("Ideal model","Conventional model","IV model")

write.csv(drmse, file = "RMSE_1000.csv")

write.csv(dpb, file = "dpb_1000.csv")
...
for (i in 1:1000){
  for (j in 1:9){
    coi[i,j] = ideal_model_cr(samp(df1000[[j]]))
    coc[i,j] = conv_model_cr(samp(df1000[[j]]))
    coiv[i,j] = IV_model_cr((samp(df1000[[j]])))
...
```{r}
iv1000=matrix(NA, nrow=1000, ncol=9)

for (i in 1:1000){
  for (j in 1:9){
    iv1000[i,j]=pbias(-0.01168, summary(IV_model(samp(df1000[[j]])))$coefficients[6])
  }
}

iv1000=data.frame(iv1000)
colnames(iv1000)=c("ex=0.2,uc=-0.07","ex=0.2,uc=-0.08","ex=0.2,uc=-0.09","ex=0.3,uc=-0.07","ex=0.3,uc=-0.08","ex=0.3,uc=-0.09","ex=0.4,uc=-0.07","ex=0.4,uc=-0.08","ex=0.4,uc=-0.09")

write.csv(iv1000, file="iv1000.csv")
