...
patientid=c(1:5000)
physicianid=sample(c(1:50))
pp=sample(c(rep(0,25),rep(1,25)))
...
df5000=list(df10_1,df20_1,df30_1,df10_2,df20_2,df30_2,df10_3,df20_3,df30_3)
...
for (i in 1:1000){
  for(j in 1:9){
    dfi[i,j]=RMSE(df5000[[j]]$W, ideal_model(samp(df5000[[j]]))$fitted.values)
    dfc[i,j]=RMSE(df5000[[j]]$W, conv_model(samp(df5000[[j]]))$fitted.values)
    dfiv[i,j]=RMSE(df5000[[j]]$W,IV_model(samp(df5000[[j]]))$fitted.values)
...
drmse_5000=data.frame(drmse)

write.csv(drmse_5000, file = "RMSE_5000.csv")
...
for (i in 1:1000){
  for(j in 1:9){
    dfi[i,j]=pbias(-0.01168, summary(ideal_model(samp(df5000[[j]])))$coefficients[7])
    dfc[i,j]=pbias(-0.01168, summary(conv_model(samp(df5000[[j]])))$coefficients[6])
    dfiv[i,j]=pbias(-0.01168, summary(IV_model(samp(df5000[[j]])))$coefficients[6])
    dpb[j,1]=mean(dfi[,j])
    dpb[j,2]=mean(dfc[,j])
    dpb[j,3]=mean(dfiv[,j])
  }
...
dpb_5000=data.frame(dpb)

write.csv(drmse_5000, file = "dpb_5000.csv")
...
colnames(drmse)=c("Ideal model","IV model","Conventional model")
...
colnames(dbias)=c("Ideal model","IV model","Conventional model")
...
colnames(dpb)=c("Ideal model","IV model","Conventional model")
