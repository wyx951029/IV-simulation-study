...
for (i in 1:1000){
  for(j in 1:9){
    dfi[i,j]=RMSE(df5000[[j]]$Y, ideal_model(samp(df5000[[j]]))$fitted.values)
    dfc[i,j]=RMSE(df5000[[j]]$Y, conv_model(samp(df5000[[j]]))$fitted.values)
    dfiv[i,j]=RMSE(df5000[[j]]$Y,IV_model(samp(df5000[[j]]))$fitted.values)
...
for (i in 1:1000){
  for (j in 1:9){
    coi[i,j] = ideal_model_cr(samp(df5000[[j]]))
    coc[i,j] = conv_model_cr(samp(df5000[[j]]))
    coiv[i,j] = IV_model_cr((samp(df5000[[j]])))
...
```{r}
iv5000=matrix(NA, nrow=1000, ncol=9)

for (i in 1:1000){
  for (j in 1:9){
    iv5000[i,j]=pbias(-0.01168, summary(IV_model(samp(df5000[[j]])))$coefficients[6])
  }
}

iv5000=data.frame(iv5000)
colnames(iv5000)=c("pp=0.3,uc=-0.07","pp=0.3,uc=-0.08","pp=0.3,uc=-0.09","pp=0.5,uc=-0.07","pp=0.5,uc=-0.08","pp=0.5,uc=-0.09","pp=0.7,uc=-0.07","pp=0.7,uc=-0.08","pp=0.7,uc=-0.09")

write.csv(iv5000, file="iv5000.csv")

```
=======
```
>>>>>>> 060483012bed6e299a7fd1430acf008e15ad19a8
